...
class Quizzes::QuizSubmissionsController < ApplicationController
  ...
  def create
    ...
    if @quiz.ip_filter && !@quiz.valid_ip?(request.remote_ip)
      ...
      @submission.add_signature(sanitized_params[:signature]) if @submission.require_physical_signature?
      if @submission.preview? || (@submission.untaken? && @submission.attempt == sanitized_params[:attempt].to_i)
        ...
        # trap error here by checking params_hash to see if any unanswered question
        # potential fix is to merge with previous attempt if it was correct
  ...
  def extensions
    ...
    if authorized_action(@submission, @current_user, :add_attempts)
      ...
      @submission.did_not_attend_training = params[:did_not_attend_training] == '1' if params[:did_not_attend_training]
  ...
  def update
    ...
    if authorized_action(@submission, @current_user, :update_scores)
      ...
      if params[:marker_signature]
        @current_user.save_signature(params[:marker_signature]) if params[:marker_signature]
      end
