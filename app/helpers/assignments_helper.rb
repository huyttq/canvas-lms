...
module AssignmentsHelper
  ...
  def assignment_submission_button(assignment, user, user_submission, has_submission_exemption)
    if assignment.expects_submission? && can_do(assignment, user, :submit)
      ...
      options = {
        ...
        disabled: user_submission && (user_submission.attempts_left == 0 || !has_submission_exemption)
      }
      tags = content_tag('button', submit_text, options)
      tags << content_tag('p', I18n.t("You need to be in our office in order to submit"), {class: "alert-error"}) unless has_submission_exemption
      tags
    end
  end
